...
- 自动识别充值行为，并据此计算每日花费。
- 卡片内以 sparkline 底图展示最近 30 天每日花费，不明显撑大卡片。
...
- 底图：`Sparkline` 组件绝对定位铺在卡片底部约 40px 高，按天画**每日花费柱状图**（不是余额）：x 按日期分布，柱高与当天 `spend` 成正比，`spend` 为 null 或 ≤ 0 的日子不画柱；充值日在柱顶（无柱则在底边）画绿色实心圆点。
- 柱色随高度变深以免遮挡上层文字：柱越高越暗（白色透明度从矮柱约 0.30 线性降到最高柱约 0.10）。
- 预警色：余额 < 2 红色、< 5 黄色，作用于整张卡的背景与边框。
- 欠费提示（火山 `arrearsBalance > 0`、阿里 `availableAmount < 0`）以小红字放在金额右侧同一行。
- 历史少于 2 行（算不出任何花费）时不渲染底图，只显示标题和金额。
